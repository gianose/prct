...
#declare db_select_regex='/^select\s+(\w|\*)+(,\s+\w+)*\s+from\s(?!where).+$/i'
...
db_do_query() {
	...
	[[ (${qry}) && $( echo ${1} | grep -P -i -q "${DML[${qry}]}" ) ]] || throw "InvalidArgument" "The provided query is improperly formated: ${1}"
...
db_do_dml() {
	...
	local rslt 
	local dml=$(echo ${1} | grep -P -i -o '^(insert|update|delete|merge)')
